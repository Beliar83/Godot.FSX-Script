...
type FsxScriptResourceFormatLoader() as this =
    ...
    override _._RecognizePath(path, _type) =
        this._HandlesType _type && FsxScriptResourceFormatLoader.extensions |> Array.contains (Path.GetExtension(path).Substring(1))        
    override _._HandlesType(_type) =
        _type.IsEmpty || _type = FsxScriptResourceFormatLoader.typeName

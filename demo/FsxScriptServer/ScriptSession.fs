...
type ScriptSession() as this =
    ...
    static member Validate(
        ...
        let isValid, checkFileResults =
            match answer with
            | None -> (false, None)
            | Some checkFileResults ->  (true, Some(checkFileResults))
        ...
        if validateFunctions then
            let functions =
                ...
                | Some value ->
                    ...
                    | None -> []
                    | Some value ->
                        match getMainModuleWithDeclarations value.Declarations with
                        | None -> failwith "todo"
        ...
        validationResult   
    member _.GetFullTypeName() =
        match info with
        | None -> ""
        | Some value -> value.ModuleNameToOpen
    ...
    member _.GetBaseType() =
        match info with
        | None -> new StringName("")
        | Some info -> new StringName(info.Extending)

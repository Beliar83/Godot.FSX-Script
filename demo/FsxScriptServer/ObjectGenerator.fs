...
module ObjectGenerator =
    ...
    let extractStateType (typ: FSharpEntity) =
        if typ.DisplayName = "State" then Some(typ) else None
    ...
    let extractNodes (contents: FSharpImplementationFileContents) =
        ...
        |> List.filter (fun (x, _) -> x.Attributes |> Seq.exists (fun x -> x.IsAttribute<NodeScriptAttribute>()))
    ...
    let isValidNodeMethod
        ...
        if not <| parameterCountIsValid then
            ...
            let stateArgument =
                (method.CurriedParameterGroups |> Seq.last |> Seq.head)
                    .Type.StripAbbreviations()
    ...
    let isValidGetPropertyListSignature
        ...
        (node: FSharpEntity)
        =
        if not <| isValidNodeMethod method state node (Exact(0)) then
            false
            ...
            method.ReturnParameter.Type.GenericArguments[0]
                .StripAbbreviations()
                .TypeDefinition
            <> state
        then
            ...
            let returnType =
                method.ReturnParameter.Type.GenericArguments[1].StripAbbreviations()

            if
                $"{returnType.TypeDefinition.AccessPath}.{returnType.TypeDefinition.DisplayName}"
            ...
            then
                ...
                if
                    $"{arrayItemItem.TypeDefinition.AccessPath}.{arrayItemItem.TypeDefinition.DisplayName}"
        ...
        | Some node, Some state ->
            ...
            for method in methods do
                ...
                let checkCustomMethod () =
                    if
                        not
                        <| isValidNodeMethod method state node.TypeDefinition ExtraParamCountCheckMode.ZeroOrMore
                    then
                ...
                if method.DisplayName.StartsWith '_' then
                    if method.DisplayName = "_Ready" then
                        ...
                        if not <| isValidGetPropertyListSignature method state node.TypeDefinition then
            ...
            let isOverride (method: FSharpMemberOrFunctionOrValue) =
                let nodeMethods =
                    (GeneratorHelper.extractMethods node) |> List.map (fun x -> x.DisplayName)
                    ...
                    { Name = state.DisplayName
                      ExportedFields =
                        [ for field in exportedFields do
                              if field.DisplayName = "_Quaternion" then
                                  ()

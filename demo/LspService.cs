...
public class LspService
{
    ...
    public static StreamJsonRpc.JsonRpc? lspClient;
    ...
    [MemberNotNullWhen(true, nameof(lspClient))]
    [MemberNotNullWhen(true, nameof(lspProcess))]
    public static async Task<bool> StartLspAsync()
    {
        ...
        lspClient = new JsonRpc(lspProcess.StandardInput.BaseStream, lspProcess.StandardOutput.BaseStream);
        InitializeResult initResult = await lspClient.InvokeAsync<InitializeResult>("initialize",
        ...
        await lspClient.InvokeAsync("fsharp/workspaceLoad",
    ...
    public static async Task ShutdownAndExit()
    {
        if (!IsLspRunning) return;
        GD.Print("Shutting down F# LSP Server");
        shuttingDown = true;
        await lspClient.InvokeAsync("shutdown");
        ...
        await lspClient.InvokeAsync("exit");
        lspProcess.Close();
        lspClient = null;
        lspProcess = null;
    }

    public static async Task ScriptOpened(Uri scriptPath, string scriptSourceCode)
    {
        ...
        if (!IsLspRunning)
        {
            ...
            QueuedActions.Enqueue(() => ScriptOpened(scriptPath, scriptSourceCode).Wait());
            return;
        }
        ...
        await lspClient.InvokeAsync("textDocument/didOpen", openTextDocumentParams);
        // if (response?.Error is not null)
        // {
    ...
    public static async Task ScriptClosed(Uri scriptPath)
    {
        if (!IsLspRunning)
        {
            if (shuttingDown)
        ...
        await lspClient.InvokeAsync("textDocument/didClose", closeTextDocumentParams);
        // if (response?.Error is not null)
        // {
    ...
    public static async Task ScriptChanged(Uri scriptPath, string scriptSourceCode)
    {
        if (!IsLspRunning)
        {
            if (shuttingDown)
        ...
        {
            await ScriptOpened(scriptPath, scriptSourceCode);
            return;
        }
        ...
        await lspClient.InvokeAsync("textDocument/didChange", changeTextDocumentParams);
        // if (response?.Error is not null)
        // {
        //     GD.PrintErr($"Could not parse script: {response.Error.Message}");
        // }

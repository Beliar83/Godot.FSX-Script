...
module getTypeNameFromIdent =
    ...
    type public TypeMatcher(matchAgainst: VariantType, propertyHint: PropertyHint, hintString: string) =
    ...
    let VariantMap: Map<string, VariantType> =
        VariantType.GetValues() |> Array.map (fun x -> (x.ToString(), x))  |> Map.ofSeq
    ...
    let rec convertFSharpTypeToVariantType (typeToConvert: FSharpType) =
        ...
        |> Option.orElseWith (fun () ->
            ...
            then
                ...
                | "Godot.NodePath" -> Some(VariantType.NodePath, PropertyHint.None, "")
                | "Godot.Variant" -> Some(VariantType.Nil, PropertyHint.None, "")
                | "Godot.GodotObject" -> Some(VariantType.Object, PropertyHint.None, "")
                | "Godot.Node" -> Some(VariantType.Object, PropertyHint.None, "")
                | _ -> None
                |> Some
            else
                None)
        |> Option.orElseWith (fun () ->
            if typeDefinition.IsArrayType then
                if typeDefinition.ArrayRank <> 1 then
                    ...
                    |> Option.orElseWith (fun () ->
                        ...
                        then
                            ...
                            | "Godot.Vector2" -> Some(VariantType.PackedVector2Array, PropertyHint.None, "")
                            | "Godot.Vector3" -> Some(VariantType.PackedVector3Array, PropertyHint.None, "")
                            | "Godot.Vector4" -> Some(VariantType.PackedVector4Array, PropertyHint.None, "")
                            | "Godot.Color" -> Some(VariantType.PackedColorArray, PropertyHint.None, "")
                            | _ -> None
                            |> Some
                        else
                            None)
                ...
                if isGodotArray typeToConvert then
                    ...
                    Some(VariantType.Dictionary, PropertyHint.None, "") |> Some
                elif isStringName typeToConvert then
                    Some(VariantType.StringName, PropertyHint.None, "") |> Some
                else

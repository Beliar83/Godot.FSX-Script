...
module ObjectGenerator =
        ...
        if not <| parameterCountIsValid then
            ...
            && 
               ...
               && (returnParameterType.GenericArguments |> Seq.head)
                ...
                ) || (returnParameterType.HasTypeDefinition && returnParameterType.TypeDefinition = state))
    ...
    let rec getMainModuleWithDeclarations (declarations : FSharpImplementationFileDeclaration list)=
        if declarations.Length <> 1
            then None
        else
            let entity, declarations = getModules declarations |> List.head
            if entity.IsFSharpModule then
                Some(entity, declarations)
            else
                getMainModuleWithDeclarations declarations

    let generateInfo (contents: FSharpImplementationFileContents) =
        
        let extractStateAndNode declarations =
            let entities =
                declarations
                |> List.choose
                    (fun x ->
            ...
            if errors |> List.isEmpty then
                Ok(state.Value, node.Value)
            else
                Error(errors)           

        let extractMethods(entity, declarations: FSharpImplementationFileDeclaration list) =
                declarations
                |> List.choose
                    (fun x ->
                        match x with
                          ...
                          [ for method in methods do
                                ...
                                let returnParameter =
                                    if returnParameter.Type.IsTupleType then
                                        let paramType =
                                            returnParameter.Type.GenericArguments |> Seq.tail |> Seq.head |> _.StripAbbreviations()
        ...
        match getMainModuleWithDeclarations contents.Declarations with
        | None -> Result.Error ["Main module not found"]
        | Some (entity, declarations) ->
            match extractStateAndNode(declarations) with
            | Ok (state, node) -> buildInfo(entity, extractMethods(entity, declarations), state, node)
